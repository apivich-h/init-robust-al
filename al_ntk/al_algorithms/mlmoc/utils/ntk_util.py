...
from . import util, gpu_util, predict, torch_to_flax
from functools import partial, lru_cache
from collections import Counter
from dppy.finite_dpps import FiniteDPP
import neural_tangents._src.utils as nt_utils
from .probability_utils import project_into_probability_simplex, project_into_probability_simplex_torch
# from .al_util import init_centers
import tqdm
...
def update_ntk_params_wo_running_stats(params, model):
    ...
    for i, (name, param) in enumerate(model.named_parameters()):
        ...
        try:
            new_jax_params.at[:].update(new_param)
        except:
...
def get_uncertainty_ntk_block_sequential(data_pool, selected, model, subset_predictions, al_params,
                                         cycle, cycle_count, ntk_config, sample_count, num_devices=1):

    labeled_dataset, unlabeled_dataset = data_pool.dataset, data_pool.dataset
    # labeled_set, subset = dataloaders['labeled_set'], dataloaders['subset']
    labeled_set = np.argwhere(selected).flatten()
    subset = np.arange(selected.shape[0])  # np.argwhere(~self.selected).flatten()

    # apply_fn, loss_fn, ntk_fn_batched, params, rng = \
    #     models['apply_fn'], models['loss_fn'], models['ntk_fn'], models['ntk_params'], models['rng']

    num_classes = model.out_dim  #model_config['model_arch']['num_classes']
    ...
    dynamic_batched_ntk_fn = lambda x1, x2: model.get_ntk(x1=x1, x2=x2, get_diagonal_only=False)
    ...
    # dynamic_batched_ntk_trace_fn = lambda x1:
    ...
    X_train = jnp.asarray(X_train.detach().cpu().numpy(), dtype=jnp.float64)  # .transpose(0, 2, 3, 1)
    y_train_onehot = np.zeros((len(y_train), num_classes))
    y_train_onehot[np.arange(len(y_train)), y_train] = 1
    y_train_onehot = jnp.array(y_train_onehot, dtype=jnp.float64)

    X_test = jnp.asarray(X_test.detach().cpu().numpy(), dtype=jnp.float64)  # .transpose(0, 2, 3, 1)
    ...
    fx_train_0 = model.call_jnp(X_train)
    fx_test_0 = model.call_jnp(X_test)
    ...
    prev_time = time.time()
    k_train_train = np.einsum('ij,kl->ijkl', np.array(dynamic_batched_ntk_fn(X_train, None)), np.eye(k))  # n x n x kn x kn
    print('train train is done: {}'.format(time.time() - prev_time), flush=True)
    prev_time = time.time()
    k_test_train = np.einsum('ij,kl->ijkl', np.array(dynamic_batched_ntk_fn(X_test, X_train)), np.eye(k)) # nt x n x knt x kn
    print('test train is done: {}'.format(time.time() - prev_time), flush=True)
    prev_time = time.time()
    k_test_test = np.array(dynamic_batched_ntk_fn(X_test, None))
    # k_test_test = np.einsum('ij,kl->ijkl', np.array(dynamic_batched_ntk_fn(X_test, None)), np.eye(k))  # nt x nt x k x k
    print('test test done: {}'.format(time.time() - prev_time), flush=True)
    ...
    if diag_reg > 0.0:  # Todo: make this compatible with trace_axes
        ...
        dimension = n * k
        batch = 20000
        A = np.array(k_train_train.reshape((n * k, n * k)))  # .transpose(0, 2, 3, 1)
        del k_train_train
        ...
        k_train_train = A.reshape(n, k, n, k)  # .transpose(0, 2, 3, 1)
        del A
    ...
    print('fx_test_0.shape =', fx_test_0.shape)
    induced_probs = project_into_probability_simplex_torch(fx_test_0)
    ...
    def compute_uncertainties(nt, n, k_train_train, k_test_train, k_test_test, induced_probs,
        ...
        for i in tqdm.trange(0, nt, step_size):  # sorry, this is because of memory issues
            ...
            # use einsum to expand to correct dimensions
            # this to avoid keeping matrices that are too large
            extra_k_train_train_point_batch = np.einsum('ij,kl->ijkl', k_test_test[i:i + step_batch_size, i:i + step_batch_size], np.eye(k))
            extra_k_test_train_col_batch = np.einsum('ij,kl->ijkl', k_test_test[:, i:i + step_batch_size], np.eye(k))
    ...
    if ntk_config.get('sequential_selection', False):
        ...
        selections = list(labeled_set)
        add_num = sample_count
        ...
        for i in range(add_num):
            ...
            k_point = np.einsum('ij,kl->ijkl', k_test_test[point:point + 1, point:point + 1], np.eye(k))
            ...
            k_row = np.einsum('ij,kl->ijkl', k_test_test[:, point:point + 1], np.eye(k))
            ...
            if ntk_config.get('sequential_with_true_label', True):
                ...
                ylabel = np.argmax(np.asarray(original_fx_test_t)[point])
            new_y_train_point[0, ylabel] = 1.
            ...
            print('Selected point - {} from class - {} with uncrt - {}, time: {}'.format(
                point, ylabel, expected_uncertainties[point], time.time() - prev_time))
    ...
    #     print('Hybrid selection finished with time: {}sec.'.format(time.time() - hybrid_prev_time))
        ...
        if use_dpp:
            sorted_points = get_sorted_points_with_dpp(
                expected_uncertainties, dpp_sim_matrix, nt, ntk_config['gamma'], sample_count)
        else:
            add_num = sample_count
