...
ensemble = EnsembleModelTorch.construct_ensemble(
    in_dim=data.input_dimensions(), out_dim=1, family='mlp',
    min_weight_to_use=1e-6, kernel_batch_sz=256, parametrisation='ntk',
    generate_jax_model=True, **model_params
)
...
for r in range(rounds):
    ...
    for i in idxs:
        ...
        m = ensemble.models[i].equiv_jax_model
        ...
        for rnd in range(100):
            ...
            train_mlp(
                model=m,
                dataset=data,
                training_subset=alg.get_selected(),
                epochs=train_epochs,
                optimiser=optz,
                learning_rate=lr,
                loss_fn='mse',
                regularisation_factor=0.,
                batch_sz=-1,
                prog_bar=False,
                record_loss_every=train_epochs,
            )
            output = np.array(m(jnp.array(data.xs_test)))
