...
for case_idx in tqdm.tqdm(CASES):
                        ...
                        if k in name.keys():
                            ...
                            if k == 'batch_rand_all':
                                ...
                                if gp == 'frgp':
                                    ...
                                    label = f"{crit}-{approx}"
                            ax.plot(xs, ys, name[k][0], color=name[k][1], alpha=0.9, label=label)
                    else:
    ...
    if PLOT_STUFF:
        ...
        plot_val_multirow(
            ...
            xs_labels=[
                f'EV criterion based on the NTKGP',
                f'MI criterion based on the NTKGP',
                'Selection points',
            ],
            ys_label='Test MSE',
            track_crit_arr=[True, True, True, True],
            gp_arr=['frgp', 'frgp', 'frgp'],
            crit_arr=['ev', 'mi', {'ev', 'mi'}],
            multicolour=False,
            get_score_arr=[True, True, False],
            add_legend_lst=[False, False, True],
            alpha=[0.5, 0.5, 0.2],
            xticks_rounds_arr=[False, False, True],
            marker='..x'
        )
        
        plot_val_multirow(
            fname_prefix='mse_mean-criterion_round-afrgp_xv',
            xs_fns=[
                lambda a, i: np.mean(a[f'r{i}_frgp_criterion_ev_tr']),
            ...
            xs_labels=[
                f'EV criterion based on the NTKGP',
                f'90V criterion based on the NTKGP',
                f'100V criterion based on the NTKGP',
                'Selection points',
            ],
            ys_label='Test MSE',
            track_crit_arr=[True, True, True, True],
            gp_arr=['frgp', 'frgp', 'frgp', 'frgp'],
            crit_arr=['ev', '90v', 'mv', {'ev', '90v', 'mv'}],
            multicolour=False,
        ...
        plot_val_multirow(
            fname_prefix='mse_mean-round-afrgp_xv',
            xs_fns=[
                lambda a, i: i
            ],
            ys_fn=lambda a, i: a[f'r{i}_test_mse_loss_final_mean'],
        ...
        plot_val_multirow(
            fname_prefix='mse_mean-criterion-afrgp',
            xs_fns=[
            ...
            add_legend_lst=[False, False],
            alpha=[0.5, 0.5],
            xticks_rounds_arr=[False, False]
        )

        plot_val_multirow(
            fname_prefix='mse_mean-criterion-afrgp_xv',
            xs_fns=[
                lambda a, i: np.mean(a[f'r{i}_frgp_criterion_ev_te']),
                lambda a, i: np.mean(a[f'r{i}_frgp_criterion_90v_te']),
                lambda a, i: np.mean(a[f'r{i}_frgp_criterion_mv_te']),
            ...
            ys_label='Test MSE',
            track_crit_arr=[False, False, False],
            gp_arr=[None, None, None],
            crit_arr=[None, None, None],
            multicolour=False,
            get_score_arr=[True, True, True],
            add_legend_lst=[False, False, False],
            alpha=[0.5, 0.5, 0.5],
            xticks_rounds_arr=[False, False, False]
        )

        plot_val_multirow(
            fname_prefix='predvar_mean-criterion_round-afrgp',
            xs_fns=[
                lambda a, i: np.mean(a[f'r{i}_frgp_criterion_ev_te']),
                lambda a, i: np.mean(a[f'r{i}_frgp_criterion_mi_te']),
                lambda a, i: i
        ...
        plot_val_multirow(
            fname_prefix='predvar_mean-criterion_round-afrgp_xv',
            xs_fns=[
                lambda a, i: np.mean(a[f'r{i}_frgp_criterion_ev_te']),
                lambda a, i: np.mean(a[f'r{i}_frgp_criterion_90v_te']),
                lambda a, i: np.mean(a[f'r{i}_frgp_criterion_mv_te']),
        ...
        for c in crits:
            plot_val_multirow(
                fname_prefix=f'predvar_mean-round-c{c}',
                xs_fns=[
                    lambda a, i: i
                ],
                ys_fn=lambda a, i: a[f'r{i}_test_pred_var_mean'],
            ...
            plot_val_multirow(
                fname_prefix=f'predvar_q90-round-c{c}',
                xs_fns=[
                    lambda a, i: i
                ],
                ys_fn=lambda a, i: a[f'r{i}_test_preds_var_q90'],
                xs_labels=[
                    'Selection points',
                ],
                ys_label='90th %ile output variance',
                track_crit_arr=[True],
                gp_arr=[None],
                crit_arr=[c],
                multicolour=False,
        ...
        for (q, ys_label) in [
            ...
            plot_val_multirow(
                fname_prefix=f'predvar_q{q}-criterion_round-afrgp_xv',
                xs_fns=[
                    lambda a, i: np.mean(a[f'r{i}_frgp_criterion_ev_te']),
                    lambda a, i: np.mean(a[f'r{i}_frgp_criterion_90v_te']),
                    lambda a, i: np.mean(a[f'r{i}_frgp_criterion_mv_te']),
            ...
            plot_val_multirow(
                fname_prefix=f'predvar_q{q}-criterion-afrgp',
                xs_fns=[
                    lambda a, i: np.mean(a[f'r{i}_frgp_criterion_ev_te']),
                    lambda a, i: np.mean(a[f'r{i}_frgp_criterion_mi_te']),
                ],
            ...
            plot_val_multirow(
                fname_prefix=f'predvar_q{q}-criterion-afrgp_xv',
                xs_fns=[
                    lambda a, i: np.mean(a[f'r{i}_frgp_criterion_ev_te']),
                    lambda a, i: np.mean(a[f'r{i}_frgp_criterion_90v_te']),
                    lambda a, i: np.mean(a[f'r{i}_frgp_criterion_mv_te']),
            ...
            plot_val_multirow(
                fname_prefix=f'predvar_q{q}-round-afrgp',
                xs_fns=[
                    lambda a, i: i,
                ],
                ys_fn=lambda a, i: a[f'r{i}_test_preds_var_q{q}'],
            ...
            plot_val_multirow(
                fname_prefix=f'predvar_q{q}-round-afrgp_xv',
                xs_fns=[
                    lambda a, i: i,
                ],
                ys_fn=lambda a, i: a[f'r{i}_test_preds_var_q{q}'],
            ...
            for c in crits.keys():
                plot_val_multirow(
                    fname_prefix=f'predvar_q{q}-criterion_round-c{c}',
                    xs_fns=[
                        lambda a, i: np.mean(a[f'r{i}_frgp_criterion_{c}_te']),
                        lambda a, i: i
                    ],
                    ys_fn=lambda a, i: a[f'r{i}_test_preds_var_q{q}'],
                    xs_labels=[
                        f'{crits[c]} criterion based on the NTKGP',
                        'Selection points',
                    ],
                    ys_label=f'{ys_label} output variance',
        ...
        for c in crits.keys():
            plot_val_multirow(
                fname_prefix=f'mse_mean-criterion_round-c{c}',
                xs_fns=[
                    lambda a, i: np.mean(a[f'r{i}_frgp_criterion_{c.lower()}_te']),
                    lambda a, i: np.mean(a[f'r{i}_sgp_criterion_{c.lower()}_te']),
                    lambda a, i: i
                ],
                ys_fn=lambda a, i: a[f'r{i}_test_mse_loss_final_mean'],
                xs_labels=[
                    f'{crits[c]} criterion based on the NTKGP',
                    f'{crits[c]} criterion based on the sNTKGP',
                    'Selection points',
                ],
                ys_label='Test MSE',
                track_crit_arr=[True, True, True],
                gp_arr=[None, None, None],
                crit_arr=[c.lower(), c.lower(), c.lower()],
                multicolour=False,
                get_score_arr=[True, True, False],
                add_legend_lst=[False, False, True],
                alpha=[0.5, 0.5, 0.2],
        ...
        plot_val_multirow(
            ...
            xs_labels=[
                'Variance predicted from NTKGP',
                'Variance predicted from sNTKGP',
            ],
        ...
        if 'classification' in params['prob_type']:
            ...
            plot_val_multirow(
                ...
                ys_label='Mean of test accuracy',
                track_crit_arr=[True],
                gp_arr=['frgp'],
                crit_arr=[{'ev', 'mi'}],
                multicolour=False,
            ...
            plot_val_multirow(
                fname_prefix=f'acc_mean-round-afrgp_xv',
                xs_fns=[
                    lambda a, i: i
                ],
                ys_fn=lambda a, i: np.mean(a[f'r{i}_test_accuracy']),
